...
import {GET,POST,POST2,FILE,FILEPOST,PUT,GETNOBASE, FILEDOWN,GETByName} from "./api";
...
export const currentGETByName = (key:Keys, connectionName?: string, param?: any) => {
  return GETByName(urlObj[key], param||{}, connectionName);
};
...
export const currentPOST2 = (key:Keys, param?: any) => {
  return POST2(urlObj[key], param||{});
};
...
export const currentFILE = (key:Keys, param?: any) => {
  return FILE({url: urlObj[key], param});
};
export const currentFILEBody = (key:Keys, body?: any) => {
  return FILEDOWN({url: urlObj[key], body});
};
// 通用接口集合
