...
export const useSettingStore = defineStore('setting', () => {
    ...
    const dateRange = ref<DateRangeDTO>({
        interval: 0
    })
    const dbArray = ref<string[]>([]);
    const addDatebase = (db: string) => {
        dbArray.value.push(db)
    }
    const getDatebase = async () => {
        if(dbArray.value.length <= 0){
            const res = await currentGET('GetDBConnection');
            console.log(res,"---")
            dbArray.value = res.result
        }
    }
    const setDateRange = (range: DateRangeDTO) => {
        if(range.team === '0'){
            range.team = '1'
        }
        if(range.group === '0'){
